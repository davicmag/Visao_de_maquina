...
mask_bg = cv2.inRange(img_hsv, lower_blue, upper_blue)

# inverter 
mask_fundo = cv2.bitwise_not(mask_bg)


# Separando vaso da planta
# ─── DETECÇÃO DO VASO + BASE POR COR (HSV) ────────────────────────────────

lower_dark = np.array([0,   0,   0])
upper_dark = np.array([180, 255, 80])
mask_vaso_escuro = cv2.inRange(img_hsv, lower_dark, upper_dark)

lower_white = np.array([0,   0, 150])
upper_white = np.array([180, 60, 255])
mask_base_branca = cv2.inRange(img_hsv, lower_white, upper_white)

mask_vaso_total = cv2.bitwise_or(mask_vaso_escuro, mask_base_branca)

h_total = mask_vaso_total.shape[0]
mask_vaso_total[:h_total // 2, :] = 0

kernel_fill = np.ones((20, 20), np.uint8)
mask_vaso_total = cv2.morphologyEx(mask_vaso_total, cv2.MORPH_CLOSE, kernel_fill)

num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(mask_vaso_total, connectivity=8)
mascara_vaso = np.zeros_like(mask_vaso_total)
for i in range(1, num_labels):
    if stats[i, cv2.CC_STAT_AREA] > 1000:
        mascara_vaso[labels == i] = 255

# Subtrai da máscara da planta
img_planta = cv2.subtract(mask_fundo, mascara_vaso)

# ─── CORTE: usa apenas o MAIOR blob do vaso para definir o topo ───────────
topo_vaso = None
maior_area = 0
...
for i in range(1, num_labels):
    ...
    if area > maior_area:
        maior_area = area
        # Topo deste blob = coordenada Y mais alta dele
        topo_vaso = stats[i, cv2.CC_STAT_TOP]

if topo_vaso is not None:
    img_planta[topo_vaso:, :] = 0

# ─── DEBUG ────────────────────────────────────────────────────────────────
plt.figure('Máscara vaso detectada')
plt.imshow(mascara_vaso, cmap='gray')
plt.title('vaso + base detectados por cor')
plt.axis('off')

plt.figure('Planta com corte na base')
plt.imshow(img_planta, cmap='gray')
plt.title('planta com corte na base')
plt.axis('off')
# ─── FIM ───────────────────────────────────────────────────────────────────
